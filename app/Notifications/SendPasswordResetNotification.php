...
class SendPasswordResetNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $mail = (new MailMessage)
            ->subject(__('api.notification.password_reset_subject'))
            ->greeting(__('api.notification.greeting'))
            ->line(__('api.notification.password_reset_intro'))
            ->line(__('api.notification.otp_code_label'))
            ->line(new HtmlString($codeHtml));

        if ($this->resetUrl) {
            $mail->action(__('api.notification.password_reset_action'), $this->resetUrl);
        }

        return $mail
            ->line(__('api.notification.password_reset_expiry'))
            ->line(__('api.notification.otp_ignore'));
    }

    public function toSms(object $notifiable): string
    {
        return __('api.notification.password_reset_sms', [
