...
class SendSecondaryContactNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $mail = (new MailMessage)
            ->subject(__('api.notification.secondary_contact_subject'))
            ->greeting(__('api.notification.greeting'))
            ->line(__('api.notification.secondary_contact_intro'))
            ->line(__('api.notification.otp_code_label'))
            ->line(new HtmlString($codeHtml));

        if ($this->verifyUrl) {
            $mail->action(__('api.notification.secondary_contact_action'), $this->verifyUrl);
        }

        return $mail
            ->line(__('api.notification.verification_expiry'))
            ->line(__('api.notification.secondary_contact_ignore'));
    }

    public function toSms(object $notifiable): string
    {
        return __('api.notification.secondary_contact_sms', [
    ...
    {
        Log::error('Secondary contact notification failed', [
